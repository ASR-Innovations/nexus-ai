 ...
 * 
 * Tests for the portfolio hook with TanStack Query integration
 */
...
describe('usePortfolio', () => {
  ...
  it('should fetch portfolio data when address is provided', async () => {
    ...
    const { result } = renderHook(() => usePortfolio({ address: '0x123' }), { wrapper });
  ...
  it('should not fetch when address is null', () => {
    ...
    const { result } = renderHook(() => usePortfolio({ address: null }), { wrapper });
  ...
  it('should not fetch when enabled is false', () => {
    ...
    const { result } = renderHook(
      () => usePortfolio({ address: '0x123', enabled: false }),
      { wrapper }
    );
  ...
  it('should cache portfolio data', async () => {
    ...
    // First render
    const { result: result1 } = renderHook(() => usePortfolio({ address: '0x123' }), { wrapper });
    ...
    const { result: result2 } = renderHook(() => usePortfolio({ address: '0x123' }), { wrapper });
